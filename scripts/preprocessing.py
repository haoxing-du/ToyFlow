...
def DataLoader(file_name,nevts=-1):
    ...
    def preprocessing(data):
        ...
        #data_flat +=np.random.uniform(0,1e-7,size=data_flat.shape)
...
def DataLoader_new(file_name,nevts=-1):
    '''
    Inputs:
    - name of the file to load
    - number of events to use
...
def Calo_prep(file_path,nevts=-1):
    ...
    energy, energy_layer, energy_voxel = DataLoader_new(file_path,nevts)
